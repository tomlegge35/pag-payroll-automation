...
const fmt = (v: number) => new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(v || 0);
...
export default function CycleSummaryPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setUser(u);
      const { data: profile } = await supabase.from('user_profiles').select('role').eq('user_id', u.id).maybeSingle();
      setRole(profile?.role || 'pag_operator');

      const [{ data: c, error: ce },{ data: r, error: re },{ data: n, error: ne }] = await Promise.all([
        supabase.from('payroll_cycles').select('*').eq('id',cycleId).maybeSingle(),
      ...
      setCycle(c); setRecords(r||[]); setNotifications(n||[]);
  ...
  const tGross = records.reduce((s,r) => s + (r.gross_pay||0), 0);
  const tNet = records.reduce((s,r) => s + (r.net_pay||0), 0);
  const tPaye = records.reduce((s,r) => s + (r.paye||0), 0);
  const tNic = records.reduce((s,r) => s + (r.ee_nic||0), 0);
  const tPension = records.reduce((s,r) => s + (r.pension_ee||0), 0);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            {cycle.status === 'approved' && !cycle.paid_confirmed_at && (
              <button onClick={handleConfirmPayment} disabled={confirmingPayment} className="px-6 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 disabled:opacity-50">
                {confirmingPayment ? 'Confirming...' : 'Confirm Payments Made'}
              </button>
            )}
            {cycle.paid_confirmed_at && (
              <span className="px-4 py-2 bg-green-100 text-green-700 rounded-full text-sm font-medium">✓ Payments Confirmed</span>
            )}
          </div>
        </div>
        ...
        <div className="bg-pag-navy text-white rounded-lg p-5 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><p className="text-blue-200 text-sm">Total PAYE</p><p className="text-2xl font-bold">{fmt(tPaye)}</p></div>
            <div><p className="text-blue-200 text-sm">Total NIC</p><p className="text-2xl font-bold">{fmt(tNic)}</p></div>
            <div><p className="text-blue-200 text-sm">Total Pension (EE)</p><p className="text-2xl font-bold">{fmt(tPension)}</p></div>
            <div><p className="text-blue-200 text-sm">Total Net Pay</p><p className="text-2xl font-bold text-green-300">{fmt(tNet)}</p></div>
          </div>
        </div>
            ...
            <div className="divide-y">
              {notifications.map(n=>(
                <div key={n.id} className="px-4 py-3 flex items-center justify-between">
                  ...
                  <span className={"text-xs px-2 py-0.5 rounded-full "+(n.email_sent_at?'bg-green-100 text-green-700':'bg-gray-100 text-gray-500')}>
                    {n.email_sent_at ? 'Sent '+new Date(n.email_sent_at).toLocaleDateString('en-GB') : 'Pending'}
                  </span>
                </div>
              ))}
            </div>
